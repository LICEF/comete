                                        ...
                                        <xsl:for-each-group select="dct:subject[@navigable = 'true'][@vocabLabel]" group-by="@vocabLabel">
                                            <xsl:sort select="current-grouping-key()" lang="$lang"/>
                                                ...
                                                <xsl:for-each select="current-group()">
                                                    ...
                                                    <li><xsl:value-of select="@vocabLabel"/><img style="margin-bottom:-1px; margin-left:8px" src="images/greyArrow.png" width="7" height="11"/><a class="RelatedLearningObjectsLink"><xsl:attribute name="href">javascript:setRequestVocConcept( '<xsl:value-of select="@rdf:resource"/>' );</xsl:attribute><xsl:value-of select="@conceptLabel"/></a></li>
                                                </xsl:for-each>
                                                </ul>
                                        </xsl:for-each-group>
                                        </ul>
